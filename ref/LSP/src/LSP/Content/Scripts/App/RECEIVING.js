...
function LoadForm() {
    ...
    //resize Width
    w = $(window).width() - 2;
    w_top = (Math.floor((w / 3) * 100) / 100) - 2;
...
function runScreenJson() {
    ...
    if ((n > 6 || n < 5)) {
        ...
        try {
            ...
            $.ajax({
                url: baseUrl + "/TB_R_CONTENT_LIST/RECEIVING_GETDATA",
                type: 'GET',
                data: {
                ...
                success: function (datajson) {
                    ...
                    request_time = (10000 - request_time) < 0 ? 0 : (10000 - request_time);
                    setTimeout("runScreenJson()", request_time);
                },
                error: function () {

                    console.log("ERROR Request : " + baseUrl + "/TB_R_CONTENT_LIST/RECEIVING_GETDATA"); //ex.message && ex.name
                    setTimeout("runScreenJson()", 10000);
                }
            });
        } catch (ex) {
...
function Navigator_Data(jdata) {
    ...
    for (var i = 0; i < jdata.length ; i++) {
        ...
        if (i <= 12) {
            _html1 = _html1 + addItem(jdata[i]);
        } else {
            _html2 = _html2 + addItem(jdata[i]);
        }
    ...
    $(".TOTAL_QTY .TOP_ITEM_SUB.val").text(_status_d + " / " + _count);
...
function addItem(jdata) {
    ...
    var _htm = "<div class='ST_LINE ITEM_" + jdata.ID + " STATUS_" + jdata.STATUS + " version_no_" + jdata.ROW_NO + "' >" +
                        "        <div class='ST_COL STT1 NO'><span>" + jdata.ROW_NO + "</span></div>" +
                        "        <div class='ST_COL STT2 INVOICE_NO'><span>" + jdata.CONTENT_NO + "</span></div>" +
                        //"        <div class='ST_COL STT3 PLAN_PALLET'><span>" + 1 + "</span></div>" +
                        //"        <div class='ST_COL STT4 ACTUAL_PALLET STATUS_" + jdata.STATUS + "'><span>" + (jdata.STATUS == "D" ? "1" : "") + "</span></div>" +
                        //"        <div class='ST_COL STT5 PC_ADDRESS'><span>" + isnull(jdata.PC_ADDRESS, '') + "</span></div>" +
