...
function LoadForm() {
    ...
    //resize Width
    w = $(window).width() - 2 - 40;
    w_top = (Math.floor((w / 20) * 100) / 100) - 2;
    ...
    $(".UNLOADING .TOP_ITEM.TOTAL_QTY").css({ "width": w_top * 4 });
    $(".UNLOADING .TOP_ITEM.TOTAL_QTY .title").css({ "width": w_top * 2});
    $(".UNLOADING .TOP_ITEM.TOTAL_QTY .val").css({ "width": w_top * 2 - 10});
    $(".UNLOADING .TOP_ITEM.TOTAL_QTY .val1").css({ "width": w_top * 2 - 10 });

    $(".UNLOADING .TOP_ITEM.TOTAL_QTY_TRUCK").css({ "width": w_top * 5});
    $(".UNLOADING .TOP_ITEM.TOTAL_QTY_TRUCK .title").css({ "width": w_top * 2.5});
    $(".UNLOADING .TOP_ITEM.TOTAL_QTY_TRUCK .val2").css({ "width": w_top * 2.5 - 10 });

    $(".UNLOADING .TOP_ITEM.TRUCK_NAME").css({ "width": w_top * 7.3 });
    $(".UNLOADING .TOP_ITEM.TRUCK_NAME .title").css({ "width": w_top * 1.3});
    $(".UNLOADING .TOP_ITEM.TRUCK_NAME .val2").css({ "width": w_top * 6 - 10 });

    //resize Height
    h = $(window).height() - 2;
    h_header = (Math.floor((h / 12) * 100) / 100) - 2;
    h_content = h - h_header - 32;
    h_item = (Math.floor((h_content / 19) * 100) / 100) - 2;
...
function runScreenJson() {
    ...
    */
        ...
        try {
            ...
            $.ajax({
                url: baseUrl + "/TB_R_CONTENT_LIST/RECEIVING5_GETDATA_DOCK",
                type: 'GET',
                data: {
                ...
                error: function () {
                    ...
                    console.log("ERROR Request : " + baseUrl + "/TB_R_CONTENT_LIST/RECEIVING5_GETDATA_DOCK"); //ex.message && ex.name
...
function Navigator_Data(jdata, jdataHeader) {
    ...
    for (var i = jdata.length; i < 18 ; i++) {
        _html1 = _html1 + addItemNone((i + 1));
    }
    _planNotScan = jdataHeader.PLAN_PALLET_QTY_NOT_SCAN
    _plan = jdataHeader.PLAN_PALLET_QTY;
    _actual = jdataHeader.ACTUAL_PALLET_QTY;
    $(".TOTAL_QTY .TOP_ITEM_SUB.val1").text(isnull(_planNotScan,""));
    $(".TOTAL_QTY .TOP_ITEM_SUB.val").text(isnull(_actual + " / " + _plan, ""));
    $(".TOTAL_QTY_TRUCK .TOP_ITEM_SUB.val2").text(isnull(jdataHeader.TRUCK_QTY_REMARK,""));
    $(".TRUCK_NAME .TOP_ITEM_SUB.val2").text(isnull(jdataHeader.TRUCK_NAME,""));
...
function addItem(jdata, no) {
    ...
    var _htm = "<div class='ST_LINE ITEM_" + jdata.ROW_NO + " STATUS_" + status + " version_no_" + jdata.ROW_NO + " ROW_NO_" + jdata.ROW_NO + "'>" +
                        ...
                        "        <div class='ST_COL STT2 DOCK_NO'><span>" + isnull(jdata.DOCK_NO, '') + "</span></div>" +
                        "        <div class='ST_COL STT3 CONTENT_NO'><span>" + jdata.CONTENT_NO + "</span></div>" +
                        "        <div class='ST_COL STT4 ETA'><span>" + jdata.ETA + "</span></div>" +
...
function addItemNone(no) {
    ...
    var _htm = "<div class='ST_LINE ITEM_ STATUS_ version_no_' >" +
                        ...
                        "        <div class='ST_COL STT2 DOCK_NO'><span> </span></div>" +
                        "        <div class='ST_COL STT3 CONTENT_NO'><span> </span></div>" +
                        "        <div class='ST_COL STT4 ETA'><span> </span></div>" +
