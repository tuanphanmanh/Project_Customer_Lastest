...
function LoadForm() {
    ...
    var w_title_left = Math.floor((w / 2.5) * 100) / 100;
    wContent = w - w_title_left;
    var w_title_txt = Math.floor((w_title_left / 14) * 100) / 100; 
    ...
    $(".HEADER .HEADER_TITLE .H_TITLE.TXT .RETURN," +     
      ".DATA .DATA_TRUCK .D_DATA.TITLE .RETURN").css({ "width": (w_title_txt * 5) - 2 });

    $(".HEADER .HEADER_TITLE .H_TITLE.TXT .SUPPLIER, " +
       ".DATA .DATA_TRUCK .D_DATA.TITLE .SUPPLIER").css({ "width": (w_title_txt * 5) - 2 });
    ...
    //TRUCK DELAY
    $(".MAIN .TRUCK_DELAY").css({ "top": (hitem - 2) + "px" });
    $(".TRUCK_DELAY .truckcolumn").css({ "height": (h_header2 - 2), "line-height": (h_header2 - 2) + "px" });
...
function runScreenJson() {
    var _Screen_name = $(".SETTINGS_SCREEN_NAME").text().trim();
    try {
        ...
        $.ajax({
            url: baseUrl + "/TB_R_UNLOADING_PLAN/UNLOADING_MAIN_SCREEN_GETDATA4_DOCK",
            type: 'GET',
            data: {
            ...
            success: function (datajson) {
                try {
                    bindData(datajson[0]);
                    bindTruckDelay(datajson[1]);
                    bindSound(datajson[0]);
                } catch (ex) {
            ...
            error: function () {
                console.log("ERROR Request : " + baseUrl + "/TB_R_UNLOADING_PLAN/UNLOADING_MAIN_SCREEN_GETDATA4_DOCK"); //ex.message && ex.name
                setTimeout("runScreenJson()", 1000);
            }
        });
    } catch (ex) {
...
function bindTruckDelay(jdata) {
    
    if (jdata.length > 0)  { $(".TRUCK_DELAY").show(); }
    else { $(".TRUCK_DELAY").hide(); }


    $(".TRUCK_DELAY .truckcontent .truckcontentblock").html("");
    for (var i = 0; i < jdata.length; i++) {
        $(".TRUCK_DELAY .truckcontent .truckcontentblock").append(addItemTRUCK_DELAY(jdata[i], (i + 1)));
    }

}
function addItemTRUCK_DELAY(jdata, no) {
    
    var _htm = '<div class="truckitem">' +
                     '     <div class="truckcolumn NO"> ' + no + ' </div>' +
                     '     <div class="truckcolumn TRUCK"> ' + jdata.TRUCK + ' </div>' +
                     '     <div class="truckcolumn RECEIVE"> ' + jdata.SUPPLIERS + ' </div>' +
                     '     <div class="truckcolumn PLAN_START_TIME"> ' + jdata.PLAN_START_UP_DATETIME_Str_DDMMYYYY + ' </div>' +
                     '     <div class="truckcolumn PLAN_FINISH_TIME"> ' + jdata.PLAN_FINISH_UP_DATETIME_Str_DDMMYYYY + ' </div>' +
                     ' </div>';
    return _htm;
}
...
function toDDMMYYY_HHMM(plan_date) {

    //Fri Apr 16 2021 07:25:00 GMT+0700 (Indochina Time)
    var dt = jsonDateToDate(plan_date);
    var hour = dt.getUTCHours();
    var min = dt.getUTCMinutes();

    var hour = ((hour + "").length == 1) ? ("0" + hour) : hour;
    var min = ((min + "").length == 1) ? ("0" + min) : min;

    var _date = dt.getUTCDate() + '/' + dt.getUTCMonth() + '/' + dt.getUTCFullYear() + ' ' +
                     hour + ':' + min; 
    return _date; 
}
/**********************************PAUSE*******************************************/
