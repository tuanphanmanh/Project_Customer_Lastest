...
function Add() {
    OnPopTB_R_PRODUCTION_PLAN_DCloseUp();
    popTB_R_PRODUCTION_PLAN_D.Show();
    return false;
}
...
function Edit(t) {
    ...
    $.ajax({
        url: baseUrl + "/TB_R_PRODUCTION_PLAN_D/TB_R_PRODUCTION_PLAN_D_Get",
        method: "post",
        dataType: "json",
        data: {
            sid: _id
        ...
        $("span[id^='popTB_R_PRODUCTION_PLAN_D']").html("Edit TB_R_PRODUCTION_PLAN_D");
        ...
        var d = new Date(new Date().toDateString() + ' ' + jsonTimeToString(response.A_IN_TIME_PLAN));
        _A_IN_TIME_PLAN.SetValue(d);

        _A_IN_DATE_ACTUAL.SetText(response.A_IN_DATE_ACTUAL_Str_DDMMYYYY);

        var d = new Date(new Date().toDateString() + ' ' + jsonTimeToString(response.A_IN_TIME_ACTUAL));
        _A_IN_TIME_ACTUAL.SetValue(d);

        _A_OUT_DATE_PLAN.SetText(response.A_OUT_DATE_PLAN_Str_DDMMYYYY);

        var d = new Date(new Date().toDateString() + ' ' + jsonTimeToString(response.A_OUT_TIME_PLAN));
        _A_OUT_TIME_PLAN.SetValue(d);

        _A_OUT_DATE_ACTUAL.SetText(response.A_OUT_DATE_ACTUAL_Str_DDMMYYYY);

        var d = new Date(new Date().toDateString() + ' ' + jsonTimeToString(response.A_OUT_TIME_ACTUAL));
        _A_OUT_TIME_ACTUAL.SetValue(d);
...
function OnBtnUpdateClicked(s) {
    ...
    $btn.addClass("saving");
    $.ajax({
        url: baseUrl + "/TB_R_PRODUCTION_PLAN_D/SaveData",
        method: "post",
        data: GetObject(),
        dataType: "json"
    }).done(function (response) {
        ActiveForm(true);
        if (response.success) {
            popTB_R_PRODUCTION_PLAN_D.Hide();
            DoSearch();
        }
        else {
...
function OnPopTB_R_PRODUCTION_PLAN_DCloseUp() {
    _id = -1
    $("span[id^='popTB_R_PRODUCTION_PLAN_D']").html("Add New TB_R_PRODUCTION_PLAN_D")
    ClearForm();
}
...
function Delete(s) {
    ...
    $btn.html("Deleting...");
    $.ajax({
        url: baseUrl + "/TB_R_PRODUCTION_PLAN_D/Delete",
        method: "post",
        dataType: "json",
        data: {
...
function Search() {
    _searchXhr = $.ajax({
        url: baseUrl + "/TB_R_PRODUCTION_PLAN_D/SetObjectInfo",
        method: "post",
        data: {            
            CFC: _txtCFC.GetText(),
            ...
            A_IN_DATE_PLAN: _txtA_IN_DATE_PLAN.GetText(),
...
function ClearForm() {
    ...
    _A_OUT_DATE_PLAN.SetText("");
    _A_OUT_TIME_PLAN.SetText("");
    _A_OUT_DATE_ACTUAL.SetText("");
    _A_OUT_TIME_ACTUAL.SetText("");
    _VERSION_NO.SetText("");
...
function ActiveForm(active) {
    ...
    _A_OUT_DATE_PLAN.SetEnabled(active);
    _A_OUT_TIME_PLAN.SetEnabled(active);
    _A_OUT_DATE_ACTUAL.SetEnabled(active);
    _A_OUT_TIME_ACTUAL.SetEnabled(active);
...
function GetObject() {
    return {
        ...
        A_IN_DATE_PLAN: ConvertDate(_A_IN_DATE_PLAN.GetText().trim()),
        A_IN_TIME_PLAN: _A_IN_TIME_PLAN.GetText().trim(),
        A_IN_DATE_ACTUAL: ConvertDate(_A_IN_DATE_ACTUAL.GetText().trim()),
        A_IN_TIME_ACTUAL: _A_IN_TIME_ACTUAL.GetText().trim(),
        A_OUT_DATE_PLAN: ConvertDate(_A_OUT_DATE_PLAN.GetText().trim()),
        A_OUT_TIME_PLAN: _A_OUT_TIME_PLAN.GetText().trim(),
        A_OUT_DATE_ACTUAL: ConvertDate(_A_OUT_DATE_ACTUAL.GetText().trim()),
        A_OUT_TIME_ACTUAL: _A_OUT_TIME_ACTUAL.GetText().trim(),
