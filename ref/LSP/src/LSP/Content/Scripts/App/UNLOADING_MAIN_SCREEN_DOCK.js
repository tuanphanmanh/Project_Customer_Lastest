...
function LoadForm() {
    ...
    var w_title_left = Math.floor((w / 5) * 100) / 100;
    wContent = w - w_title_left;
    var w_title_txt = Math.floor((w_title_left / 6) * 100) / 100;
    ...
    $(".HEADER .HEADER_TITLE .H_TITLE.TXT .DOCK," +
       ...
       ".DATA .DATA_TRUCK .D_DATA.TITLE .DOCK span, " +
       ".DATA .DATA_TRUCK .D_DATA.TITLE .TRUCK").css({ "width": w_title_txt - 2 });
    
    $(".HEADER .HEADER_TITLE .H_TITLE.TXT .SUPPLIER, " +
       ".HEADER .HEADER_TITLE .H_TITLE.TXT .RETURN, " +
       ".DATA .DATA_TRUCK .D_DATA.TITLE .SUPPLIER, " + 
       ".DATA .DATA_TRUCK .D_DATA.TITLE .RETURN").css({ "width": (w_title_txt * 2) - 2 });
    ...
    hitem = Math.floor((hContent / 20) * 100) / 100;
...
function runScreenJson() {
    ...
    try {
        ...
        $.ajax({
            url: baseUrl + "/TB_R_UNLOADING_PLAN/UNLOADING_MAIN_SCREEN_GETDATA_DOCK",
            type: 'GET',
            data: {
            ...
            error: function () {
                console.log("ERROR Request : " + baseUrl + "/TB_R_UNLOADING_PLAN/UNLOADING_MAIN_SCREEN_GETDATA_DOCK"); //ex.message && ex.name
                setTimeout("runScreenJson()", 1000);
            }
        });
    } catch (ex) {
...
function addItemHTML(jdata, _key) { 
    var _htm = '<div class="item ' + _key + ' STATUS_' + jdata.STATUS + ' " >' +
                    '    <div class="itemsub">' +
                    '       <span></span>' +
                    '    </div>' +
                    '</div>';
