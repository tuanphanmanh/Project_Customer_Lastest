...
namespace TMV.Common
{
    public class Commons
    {
        ...
        public static void DrawVerticalText(object sender, PaintEventArgs e, string text)
        {
            LabelControl lbl = (LabelControl)sender;
            SolidBrush b = new SolidBrush(lbl.ForeColor);
            SizeF s = e.Graphics.MeasureString(text, lbl.Font);
            int newX = (int)(e.ClipRectangle.Width - s.Height) / 2;
            int newY = (int)(e.ClipRectangle.Height + s.Width) / 2;

            e.Graphics.TranslateTransform(newX, newY);
            e.Graphics.RotateTransform(-90);
            e.Graphics.DrawString(text, lbl.Font, b, 0, 0);
        }
