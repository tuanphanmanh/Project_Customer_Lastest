...
{
    public partial class lsp_unpacking : System.Web.UI.Page
    {
        #region DECLARATION     
        ...
        {            
            try
            {
                DataSet dsCheck = SCANNING_BIZBO.Instance().PROCESS_SCANNING_UP_CONTENT(Convert.ToString(p_value), p_user_id, p_process_id);
        ...
        private bool Validate_data_UP_PART(string p_value, string p_content_no)
        {
            try
            {
                ...
                DataSet dsCheck = SCANNING_BIZBO.Instance().PROCESS_SCANNING_UP_PART(Convert.ToString(p_value), p_content_no, p_user_id, p_process_id);
                string p_result = Convert.ToString(dsCheck.Tables[0].Rows[0][0]);
        ...
        private bool Ad_ADHOC_KANBAN(string p_content_no, string p_part_no)
        {
            try
            {
                return SCANNING_BIZBO.Instance().Ad_ADHOC_KANBAN(p_content_no, p_part_no, p_user_id, p_process_id);
