    ...
    private async Task CheckInternal()
    {
      ...
      try
      {
        if (!zeroLicenseChecker.IsProjectNameValid())
          throw new AspNetZeroLicenseException("Failed to validate project name. Should not rename a project downloaded from aspnetzero.com. You can contact to info@aspnetzero.com if you are using a licensed esignuct.");
        if (zeroLicenseChecker.CheckedBefore())
          return;
      }
      catch (Exception ex)
      {
        zeroLicenseChecker.Logger.Fatal("Failed to validate project name. Should not rename a project downloaded from aspnetzero.com. You can contact to info@aspnetzero.com if you are using a licensed esignuct." + Environment.NewLine + ex.Message, ex);
        Environment.Exit(-42);
      }
