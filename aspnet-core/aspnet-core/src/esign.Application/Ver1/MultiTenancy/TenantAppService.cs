...
namespace esign.MultiTenancy.Ver1
{
    ...
    public class TenantAppService : esignVersion1AppServiceBase, ITenantAppService
    {
        ...
        [HttpGet]
        public async Task<TenantEditDto> GetTenantForEdit([FromQuery] EntityDto input)
        {
        ...
        [HttpPost]
        public async Task DeleteTenant([FromQuery] EntityDto input)
        {
            var tenant = await TenantManager.GetByIdAsync(input.Id);
