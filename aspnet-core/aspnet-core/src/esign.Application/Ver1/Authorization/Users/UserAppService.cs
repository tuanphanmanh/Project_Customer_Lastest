...
namespace esign.Authorization.Users.Ver1
{
    ...
    public class UserAppService : esignVersion1AppServiceBase, IUserAppService
    {
    ...
    [HttpPost]
        public async Task UpdateDefautAvatarJob()
        {
            try
            {
                List<User> users;
                ...
                {
                    users = await _userManager.Users.Where(e => string.IsNullOrWhiteSpace(e.ImageUrl)).ToListAsync();
                }
                // Now 'users' holds a list of users with a blank ImageUrl.
        ...
        public string ConvertShortnameToCreateDefaultAvatar(string fullName)
        {
            ...
            string username = nameParts.Length >= 2 ?nameParts[0].Substring(0, 1) + nameParts[1].Substring(0, 1): nameParts[0].Substring(0, 1);
        ...
        [HttpGet]
        public async Task<FileDto> GetUsersToExcel([FromQuery] GetUsersToExcelInput input)
        {
            var query = GetUsersFilteredQuery(input);
