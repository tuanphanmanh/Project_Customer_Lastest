...
{
    public class StripeGatewayManager : esignServiceBase,
        ...
        ITransientDependency
    {
        ...
        public static string esignuctName = "esign";
        ...
        public async Task<StripeIdResponse> GetOrCreatePlanAsync(string planId, decimal amount, string interval, string esignuctId)
        {
            try
            {
                var planService = new PlanService();
            ...
            {
                return await CreatePlanAsync(planId, amount, interval, esignuctId);
            }
        }

        public async Task<StripeIdResponse> GetOrCreateesignuctAsync(string esignuctId)
        {
            try
            {
                var esignuctService = new ProductService();
                var esignuct = await esignuctService.GetAsync(esignuctId);
            ...
            catch (StripeException exception)
            {
                ...
                return await CreateesignuctAsync(esignuctId);
            }
        }
        ...
        public void HandleEvent(TenantEditionChangedEventData eventData)
        {
            ...
            {
                AsyncHelper.RunSync(() => CreatePlanAsync(newPlanId, newPlanAmount.Value, GetPlanInterval(subscriptionPayment.PaymentPeriodType), esignuctName));
            }
        ...
        {
            return editionName + "_" + paymentPeriodType + "_" + esignConsts.Currency;
        }
        ...
        private async Task<StripeIdResponse> CreateesignuctAsync(string name)
        {
            var esignuctService = new ProductService();
            var esignuct = await esignuctService.CreateAsync(new ProductCreateOptions
            {
                Id = name,
                Name = name,
                Type = "service"
        ...
        public async Task<StripeIdResponse> GetOrCreatePlanForPayment(long paymentId)
        {
            ...
            var esignuct = await GetOrCreateesignuctAsync(esignuctName);
            var planInterval = GetPlanInterval(payment.PaymentPeriodType);


            return await GetOrCreatePlanAsync(planId, amount, planInterval, esignuct.Id);
        }
    }
}
