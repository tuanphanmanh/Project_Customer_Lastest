...
namespace esign.Authorization.Users
{
    ...
    public class UserEmailer : esignServiceBase, IUserEmailer, ITransientDependency
    {
        ...
        private string _emailButtonStyle =
            "padding-left: 30px; padding-right: 30px; padding-top: 12px; padding-bottom: 12px; color: #ffffff; background-color: #00bb77; font-size: 14pt; text-decoration: none; border-radius: 5px;";
        ...
        public async Task SendPasswordResetLinkAsync(User user, string link = null)
        {
            ...
            if (!link.IsNullOrEmpty())
            {
                ...
                mailMessage.AppendLine("<a align=\"center\" style=\"" + _emailButtonStyle + "\" bg-color=\"" + _emailButtonColor +
                                       "\" href=\"" + link + "\">" + "Reset" + "</a>");
                mailMessage.AppendLine("<br />");
                mailMessage.AppendLine("<br />");
                mailMessage.AppendLine("<br />");
                mailMessage.AppendLine("<span style=\"font-size: 9pt;\">" +
                                       "Or follow link" + "</span><br />");
                mailMessage.AppendLine("<span style=\"font-size: 8pt;\">" + link + "</span>");
            }

            await ReplaceBodyAndSendAsync(user.EmailAddress, "Email Reset Password", emailTemplate,
                mailMessage);
        }
        ...
        public async Task<string> SetBodyForSendEmailActive(User user, string link, string plainPassword = null)
        {
            var mailMessage = new StringBuilder();
            await _unitOfWorkManager.WithUnitOfWorkAsync(async () =>
            {
        ...
        public async Task<string> SetBodyForSendEmailForgotPassword(User user, string link, string plainPassword = null)
        {
            var expirationHours = await _settingManager.GetSettingValueAsync<int>(
                AppSettings.UserManagement.Password.PasswordResetCodeExpirationHours
            );
