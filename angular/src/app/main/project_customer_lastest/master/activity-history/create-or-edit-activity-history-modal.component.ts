...
import { FileParameter, MstActivityHistoryDto, MstActivityHistoryServiceProxy } from '@shared/service-proxies/service-proxies';
...
@Component({
    selector: 'create-or-edit-activity-history-modal',
    templateUrl: './create-or-edit-activity-history-modal.component.html'
})
export class CreateOrEditActivityHistoryComponent extends AppComponentBase  implements OnInit {
    @ViewChild('createOrEditActivityHistory', { static: true }) modal: ModalDirective | undefined;
    ...
    dialogHeader = this.l('CreateActivityHistory');
    rowdata: MstActivityHistoryDto = new MstActivityHistoryDto()
    ...
    showModalCreateEdit(rowdata?: any): void {
        if (!rowdata) {
            this.dialogHeader = this.l('CreateActivityHistory');
            this.rowdata = new MstActivityHistoryDto();
        } else {
            this.rowdata = rowdata;
            this.dialogHeader = this.l('UpdateActivityHistory')
        }
        this.buildForm();
        this.modal.show();
    }
    ...
    createOrEdit(code: string | undefined,
        description: string | undefined,
        localName: string | undefined,
        internationalName: string | undefined,
        image: FileParameter | undefined,
        id: number | undefined) {
        let url_ = AppConsts.remoteServiceBaseUrl + "/api/services/app/v1/MstActivityHistory/CreateOrEdit";
        url_ = url_.replace(/[?&]$/, "");
