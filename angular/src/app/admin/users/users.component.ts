...
export class UsersComponent extends AppComponentBase implements AfterViewInit {
    ...
    revokeToken(user: UserListDto)
    {
        this.message.confirm(this.l('RevokeTokenWarningMessage'), this.l('AreYouSure'), (isConfirmed) => {
            if (isConfirmed) {
                this._tokenAuthService.removeTokenByUserName(user.userName).subscribe(() => {
                    this.notify.success(this.l('RevokeTokenSuccess'));
                });
            }
        })
    }
