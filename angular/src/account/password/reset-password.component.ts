...
export class ResetPasswordComponent extends AppComponentBase implements OnInit {
    ...
    ngOnInit(): void {
        ...
        if (this._activatedRoute.snapshot.queryParams['inputCode']) {
            this.model.inputCode = this._activatedRoute.snapshot.queryParams['inputCode'];

            this._accountService
                .resolveTenantId(new ResolveTenantIdInput({ c: this.model.inputCode }))
                .subscribe((tenantId) => {
        ...
        this._accountService
            ...
            .subscribe((result: ResetPasswordOutput) => {
                ...
                let recaptchaCallback = (token: string) => {
                    // Autheticate
                    this.saving = true;
                    this._loginService.authenticateModel.userName = result.userName;
