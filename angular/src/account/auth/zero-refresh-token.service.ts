import { RefreshTokenInput } from './../../shared/service-proxies/service-proxies';
...
export class ZeroRefreshTokenService implements RefreshTokenService {
    ...
    tryAuthWithRefreshToken(): Observable<boolean> {
        ...
        let token = new RefreshTokenInput({ refreshToken : this._tokenService.getRefreshToken()});
        if (!token || token?.refreshToken?.trim() === '' || !token?.refreshToken) {
            return of(false);
        }
